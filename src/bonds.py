...
def get_remove_bonds_keyboard():
    ...
    keyboard = []
    for bond in data.keys():
        keyboard.append([InlineKeyboardButton("Remove " + bond, callback_data="remove_bond_" + bond)])
...
async def handle_new_bond_name_input(
    ...
    await update.message.reply_text(
        "Select the new bond description"
    )
...
bonds_handler = ConversationHandler(
    ...
    states={
        ...
        WAITING_NEW_BOND_DESCRIPTION: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, handle_new_bond_description_input)
        ],
